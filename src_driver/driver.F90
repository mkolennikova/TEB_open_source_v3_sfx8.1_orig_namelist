...
CHARACTER(LEN=*), PARAMETER       :: EMIT_LW_GRND = 'output/EMIT_LW_GRND.txt'         !
CHARACTER(LEN=*), PARAMETER       :: EMIT_LW_FAC = 'output/EMIT_LW_FAC.txt'           !
CHARACTER(LEN=*), PARAMETER       :: REF_SW_GRND = 'output/REF_SW_GRND.txt'           !
CHARACTER(LEN=*), PARAMETER       :: REF_SW_FAC = 'output/REF_SW_FAC.txt'             !
CHARACTER(LEN=*), PARAMETER       :: AZIM = 'output/AZIM.txt'                         !
CHARACTER(LEN=*), PARAMETER       :: ZENITH = 'output/ZENITH.txt'                     !
INTEGER                           :: fu, rc
...
!
!===========================================================================
! internal temperature and humidity for initialization
!===========================================================================
...
!
!
ZT_ROAD  (:,1) = ZTA(1,:)    ! road layers temperatures
ZT_ROAD  (:,2) = ZTA(1,:) + ((2.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! road layers temperatures
ZT_ROAD  (:,3) = ZTA(1,:) + ((3.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! road layers temperatures
ZT_ROAD  (:,4) = ZTA(1,:) + ((4.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! road layers temperatures
ZT_ROAD  (:,5) = ZTI_BLD     ! road layers temperatures
ZT_ROOF  (:,1) = ZTA(1,:)    ! roof layers temperatures
ZT_ROOF  (:,2) = ZTA(1,:) + ((2.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! roof layers temperatures
ZT_ROOF  (:,3) = ZTA(1,:) + ((3.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! roof layers temperatures
ZT_ROOF  (:,4) = ZTA(1,:) + ((4.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! roof layers temperatures
ZT_ROOF  (:,5) = ZTI_BLD     ! roof layers temperatures
ZT_WALL_A(:,1) = ZTA(1,:)    ! wall layers temperatures
ZT_WALL_A(:,2) = ZTA(1,:) + ((2.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! wall layers temperatures
ZT_WALL_A(:,3) = ZTA(1,:) + ((3.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! wall layers temperatures
ZT_WALL_A(:,4) = ZTA(1,:) + ((4.-1.)/(5.-1.)*(ZTI_BLD - ZTA(1,:)))  ! wall layers temperatures
ZT_WALL_A(:,5) = ZTI_BLD     ! wall layers temperatures
ZT_FLOOR (:,1) = ZTI_BLD     ! building floor temperature
ZT_FLOOR (:,2) = ZTI_BLD     ! building floor temperature
ZT_FLOOR (:,3) = ZTI_BLD     ! building floor temperature
ZT_FLOOR (:,4) = ZTI_BLD     ! building floor temperature
ZT_FLOOR (:,5) = ZTI_BLD     ! building floor temperature
ZT_MASS  (:,1) = ZTI_BLD     ! building mass temperature
ZT_MASS  (:,2) = ZTI_BLD     ! building mass temperature
ZT_MASS  (:,3) = ZTI_BLD     ! building mass temperature
ZT_MASS  (:,4) = ZTI_BLD     ! building mass temperature
ZT_MASS  (:,5) = ZTI_BLD  ! building mass temperature

ZT_CANYON      = ZTA(1,:)   ! canyon air temperature
ZT_WIN1        = ZTA(1,:)   ! External window temperature
ZT_WIN2        = ZTI_BLD    ! Internal window temperature
ZQ_CANYON      = ZQA(1,:) / XRHOA   ! Outdoor specific humidity    [kg kg-1]
...
! -----------------------------------------------------------
!
ZZ0           = 0.1 * ZBLD_HEIGHT  ! Roughness length (m)
...
OPEN(UNIT=27, FILE = EMIT_LW_GRND, ACCESS = 'APPEND',STATUS = 'REPLACE')
OPEN(UNIT=28, FILE = EMIT_LW_FAC,  ACCESS = 'APPEND',STATUS = 'REPLACE')
OPEN(UNIT=29, FILE = REF_SW_GRND,  ACCESS = 'APPEND',STATUS = 'REPLACE')
OPEN(UNIT=30, FILE = REF_SW_FAC,   ACCESS = 'APPEND',STATUS = 'REPLACE')
OPEN(UNIT=31, FILE = AZIM,         ACCESS = 'APPEND',STATUS = 'REPLACE')
OPEN(UNIT=32, FILE = ZENITH,       ACCESS = 'APPEND',STATUS = 'REPLACE')
!
! -----------------------------------------------------------
! Temporal loops
! -----------------------------------------------------------
...
ZTS_WALL_B = ZT_WALL_B(1,1)
   ...
   IF (CBEM=='BEM') THEN
     ...
     WRITE(26,*) ZHVAC_HEAT
   END IF
   WRITE(27,*) ZEMIT_LW_GRND
   WRITE(28,*) ZEMIT_LW_FAC
   WRITE(29,*) ZREF_SW_GRND
   WRITE(30,*) ZREF_SW_FAC
   WRITE(31,*) XAZIM
   WRITE(32,*) XZENITH  
!
END DO
